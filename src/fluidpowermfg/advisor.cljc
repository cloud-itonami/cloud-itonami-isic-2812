(ns fluidpowermfg.advisor
  "FluidPowerAdvisor -- the *contained intelligence node* for the
  hydraulic/pneumatic fluid-power-equipment plant-operations
  coordination actor.

  It normalizes production-batch patches (product-type/pressure-test-
  bar/quantity/defect-rate), drafts a machining/assembly/pressure-
  test-bench-equipment maintenance scheduling proposal against a piece
  of equipment, drafts a safety-concern flag, and drafts an outbound
  hydraulic/pneumatic-equipment shipment coordination proposal against
  ...
  never a committed record and NEVER a real machining/assembly-line-
  equipment actuation, freight dispatch, or pressure-safety
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `fluidpowermfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `fluidpowermfg.governor`)
  ...
  `fluidpowermfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  pressure-test-bar, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a machining/assembly/pressure-test-bench-equipment
  ...
  registered?) in its rationale, but `fluidpowermfg.governor` NEVER
...
(defn- flag-safety-concern
  "Draft a pressure-rating/hydraulic-fluid-safety concern. ALWAYS
  ...
  `fluidpowermfg.phase`: no phase ever adds this op to a phase's
  `:auto` set; `fluidpowermfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound hydraulic/pneumatic-equipment shipment
  coordination proposal against a production batch. The advisor passes
  through the caller's own claimed unit quantity -- it does NOT invent
  one, and `fluidpowermfg.governor` NEVER trusts it: it independently
  recomputes whether the batch's own cumulative-shipped quantity plus
  this claim would exceed the batch's own recorded quantity before any
  commit is possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたは油圧・空気圧機器(油圧ポンプ・油圧シリンダ・油圧バルブ・油圧"
       "モータ・空気圧シリンダ・空気圧バルブ等の流体動力機器)製造工場プラント"
       "運用コーディネーターの助言者です。"
       ...
       "機械加工・組立・圧力試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "圧力機器適合性(ASME BPVC/PED 2014/68/EU等)・流体動力システム安全適合性"
       "(ISO 4413/ISO 4414等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `fluidpowermfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :fluidpowermfg-advisor-proposal
   :op         (:op request)
