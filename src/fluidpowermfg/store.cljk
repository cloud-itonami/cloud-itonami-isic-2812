(ns fluidpowermfg.store
  "SSoT for the fluid-power-equipment (hydraulic/pneumatic pumps,
  cylinders, valves, motors) plant-operations coordination actor,
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2814`'s own
  `beargearmfg.store`, `cloud-itonami-isic-2818`'s own
  ...
  is self-contained (no external fluid-power-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/pressure-test-bar/quantity/
    ...
    - `equipment`         -- a machining-line/assembly-line/pressure-
                             test-bench unit's own record.
    ...
    - `maintenance`       -- a scheduled machining/assembly/pressure-
                             test-bench-equipment maintenance-window
                             DRAFT against a piece of equipment
                             (`fluidpowermfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound fluid-power-equipment
                             shipment DRAFT (`fluidpowermfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (fluidpowermfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (fluidpowermfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :hydraulic-pump
                :model "HP-A320" :pressure-test-bar 350.0
                :quantity-units 5000.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :hydraulic-cylinder
                :model "HC-T63" :pressure-test-bar 210.0
                :quantity-units 1200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 1150.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :pneumatic-cylinder
                :model "PC-R25" :pressure-test-bar 12.0
...
(defn- sample-equipment []
  {"mach-001" {:id "mach-001" :kind :machining-line
               :verified? true :registered? true
               :last-maintenance-date "2026-05-01"}
   "bench-002" {:id "bench-002" :kind :pressure-test-bench
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `fluidpowermfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `fluidpowermfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  machining-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered pressure-test-bench unit (blocks any
