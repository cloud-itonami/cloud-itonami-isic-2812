(ns fluidpowermfg.operation
  "FluidPowerOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FluidPowerAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Fluid Power
  Equipment Plant Operations Governor (:govern) and the rollout phase
  ...
  (:require [langgraph.graph :as g]
            ...
            [fluidpowermfg.advisor :as advisor]
...
(defn build
  "Compiles a FluidPowerOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `fluidpowermfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FluidPowerAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Fluid Power Equipment Plant Operations Governor -- independent censor.
      (g/add-node :govern
