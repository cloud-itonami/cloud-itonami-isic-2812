(ns fluidpowermfg.sim
  ...
  a proposal that tries to self-issue a pressure-equipment conformity
  or fluid-power-system safety compliance mark (permanently blocked,
  no override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product type, a
  production-batch patch with an implausible pressure-test-bar
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on mach-001 (verified, registered, machining line -- escalates, approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:product-type :hydraulic-pump}}
                       coordinator))
    ...
    (println "== schedule-maintenance mnt-2 on bench-002 (UNVERIFIED/unregistered pressure-test bench -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with an implausible pressure-test-bar -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:pressure-test-bar 999999.0}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a pressure-safety certification -> HARD hold, PERMANENT ==")
