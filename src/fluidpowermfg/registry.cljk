(ns fluidpowermfg.registry
  "Pure-function domain logic for the fluid-power-equipment (hydraulic/
  pneumatic pumps, cylinders, valves, motors) plant-operations
  coordination actor -- equipment/batch verification, shipment-quantity
  recompute, product-type validation, pressure-test plausibility
  ...
  pre-existing `kotoba-lang/fluidpowermfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `fluidpowermfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `beargearmfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2814`, and `powertoolmfg.registry` from
  ...
  coordinated shipment), not the act of actuating machining/assembly-
  line equipment or dispatching a real freight carrier, and never the
  act of issuing a pressure-equipment conformity or fluid-power-system
  safety compliance mark (this actor NEVER does any of those -- see
  README `What this actor does NOT do`).

  SCOPE: ISIC 2812 covers manufacture of fluid power equipment --
  machining, assembly, and pressure-test-bench lines producing finished
  hydraulic pumps, hydraulic cylinders, hydraulic valves, hydraulic
  motors, pneumatic cylinders and pneumatic valves. This actor
  ...
  assembly-line equipment directly, and it never stands in for the
  certification body that issues pressure-equipment conformity or
  fluid-power-system safety compliance marks (e.g. ASME BPVC / PED
  2014/68/EU / ISO 4413 / ISO 4414).")
...
(def valid-product-types
  ...
  through."
  #{:hydraulic-pump :hydraulic-cylinder :hydraulic-valve :hydraulic-motor
    :pneumatic-cylinder :pneumatic-valve})

(def pressure-test-bar-min
  "Physical floor for a batch's own pressure-test reading, in bar. A
  measured test pressure is never negative."
  0.0)

(def pressure-test-bar-max
  "Physical ceiling for a batch's own pressure-test reading, in bar.
  Informed by real hydraulic/pneumatic fluid-power test practice: rated
  system pressures for high-pressure hydraulic equipment commonly run
  up to roughly 700 bar, and proof/pressure tests are typically
  performed at 1.5x-2x the rated working pressure (ISO 4413 hydraulic
  / ISO 4414 pneumatic fluid-power-system practice) -- this ceiling
  generously bounds well above any real routine pressure-test reading
  on any standardized class of hydraulic or pneumatic equipment; a
  reading above it is implausible sensor/QC data, not a real
  measurement."
  2000.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own machining/assembly/pressure-test
  defect-rate reading (zero defects is the best possible outcome,
  never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own machining/assembly/pressure-test
...
(defn batch-verified?
  ...
  (i.e. its product-type/pressure-test-bar/quantity/defect-rate claims
...
(defn pressure-test-bar-valid?
  "Is `bar` a physically plausible pressure-test reading, in bar?
  Rejects nil, non-numbers, negative values, and values beyond
  `pressure-test-bar-max` -- a fabricated or sensor-error reading,
  never let through as a real test-result fact."
  [bar]
  (and (number? bar)
       (>= (double bar) pressure-test-bar-min)
       (<= (double bar) pressure-test-bar-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch machining/assembly/
  pressure-test defect-rate reading? Rejects nil, non-numbers, negative
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a pressure-equipment conformity or fluid-power-
  system safety compliance mark -- this actor is never the
...
(defn register-maintenance
  ...
  machining/assembly/pressure-test-bench-equipment maintenance window
  against a verified, registered piece of equipment. Pure function --
  does not actuate the machining/assembly-line equipment or execute
  any maintenance; it builds the RECORD a plant coordinator would
  keep. `fluidpowermfg.governor` independently re-verifies the
...
(defn register-shipment
  ...
  outbound fluid-power-equipment shipment against a verified,
