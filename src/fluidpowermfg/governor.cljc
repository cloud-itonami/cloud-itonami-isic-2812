(ns fluidpowermfg.governor
  "Fluid Power Equipment Plant Operations Governor -- the independent
  compliance layer that earns the FluidPowerAdvisor the right to
  ...
  draft-schedule) machining/assembly-line equipment, whether a
  proposal secretly tries to self-issue a pressure-equipment
  conformity or fluid-power-system safety compliance mark (an
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       pressure-equipment conformity or
                                       fluid-power-system safety
                                       compliance mark (e.g. ASME BPVC
                                       / PED 2014/68/EU / ISO 4413 /
                                       ISO 4414) -- an authority
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`fluidpowermfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid pressure-test-bar   -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:pressure-test-bar` that is not
                                       a physically plausible
                                       pressure-test reading
                                       (`fluidpowermfg.registry/
                                       pressure-test-bar-valid?`), the
   ...
   12. Invalid defect-rate         -- for `:log-production-batch`, if
                                       ...
                                       reading (`fluidpowermfg.registry/
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [fluidpowermfg.registry :as registry]
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct machining/assembly-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は機械加工・組立設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate machining/assembly-line equipment -- this actor
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "機械加工・組立・圧力試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a pressure-equipment conformity or fluid-power-system
  safety compliance mark (e.g. ASME BPVC / PED 2014/68/EU / ISO 4413 /
  ISO 4414) -- an authority exclusively reserved to the accredited
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "圧力機器適合性(ASME BPVC/PED 2014/68/EU等)・流体動力システム安全適合性(ISO 4413/ISO 4414等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-pressure-test-bar-violations
  "For `:log-production-batch`, if the patch declares a
  `:pressure-test-bar` that is not a physically plausible pressure-test
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [bar (:pressure-test-bar (:value proposal))]
      (when (and (some? bar) (not (registry/pressure-test-bar-valid? bar)))
        [{:rule :invalid-pressure-test-bar
          :detail (str bar "bar は物理的に妥当な圧力試験値の範囲外")}]))))
...
(defn check
  "Censors a FluidPowerAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-pressure-test-bar-violations request proposal)
