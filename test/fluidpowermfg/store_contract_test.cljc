(ns fluidpowermfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `fluidpowermfg.store` ns docstring for why a second (Datomic-
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (= ["bench-002" "mach-001"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :hydraulic-cylinder}})
    (is (= :hydraulic-cylinder (:product-type (store/batch s "batch-001"))))
