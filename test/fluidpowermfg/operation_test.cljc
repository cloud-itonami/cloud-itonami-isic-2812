(ns fluidpowermfg.operation-test
  "Smoke tests for the compiled FluidPowerOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `fluidpowermfg.governor-contract-test`; the Store contract in
  `fluidpowermfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FluidPowerOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "mach-001" :severity :moderate :description "hydraulic-line pressure anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
