(ns fluidpowermfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control machining/assembly-line equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue a pressure-equipment conformity or
  fluid-power-system safety compliance mark') implemented faithfully.
  The single invariant under test:

    FluidPowerAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Fluid Power Equipment Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :hydraulic-pump}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :hydraulic-pump (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the pressure-safety certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-pressure-test-bar-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:pressure-test-bar 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-pressure-test-bar} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:pressure-test-bar (store/batch db "batch-001"))) "fabricated pressure-test-bar never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "mach-001" :severity :moderate
                                            :description "hydraulic-line pressure anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
