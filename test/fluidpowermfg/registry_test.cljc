(ns fluidpowermfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [fluidpowermfg.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [pt [:hydraulic-pump :hydraulic-cylinder :hydraulic-valve :hydraulic-motor
...
;; ----------------------------- pressure-test-bar-valid? -----------------------------

(deftest typical-pressure-test-bar-is-valid
  (is (r/pressure-test-bar-valid? 0.0))
  (is (r/pressure-test-bar-valid? 12.0))
  (is (r/pressure-test-bar-valid? 350.0))
  (is (r/pressure-test-bar-valid? 2000.0)))

(deftest negative-pressure-test-bar-is-invalid
  (is (not (r/pressure-test-bar-valid? -1.0))))

(deftest excessive-pressure-test-bar-is-invalid
  (is (not (r/pressure-test-bar-valid? 999999.0)))
  (is (not (r/pressure-test-bar-valid? 2000.01))))

(deftest non-numeric-or-missing-pressure-test-bar-is-invalid
  (is (not (r/pressure-test-bar-valid? nil)))
  (is (not (r/pressure-test-bar-valid? "350.0"))))
